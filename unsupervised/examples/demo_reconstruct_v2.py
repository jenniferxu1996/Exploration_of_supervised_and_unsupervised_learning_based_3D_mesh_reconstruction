...
def main(_args=None):
    ...
    if args.load:
        model.load_state_dict(torch.load(os.path.join(args.output_dir, 'v2-%d.pth' % args.train_num)))
    ...
    if not args.load:
        ...
        torch.save(model.state_dict(), os.path.join(args.output_dir, 'v2-%d.pth' % args.train_num))
