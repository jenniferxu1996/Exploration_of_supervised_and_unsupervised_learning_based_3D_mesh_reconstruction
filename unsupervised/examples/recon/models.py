...
class Decoder(nn.Module):
    def __init__(self, filename_obj, centroid_scale=0.1, bias_scale=1.0, centroid_lr=0.1, bias_lr=1.0):
        super(Decoder, self).__init__()
        # load .obj
        self.template_mesh = sr.Mesh.from_obj(filename_obj)
        # vertices_base, faces = srf.load_obj(filename_obj)
        ...
        self.deconv2 = nn.ConvTranspose2d(256, 128, 3, stride=2)
        self.deconv3 = nn.ConvTranspose2d(128, 64, 3, stride=2)
        self.fc2 = nn.Linear(23*23*64, 2048)

        self.fc_centroid = nn.Linear(2048, 3)
        self.fc_bias = nn.Linear(2048, self.template_mesh.num_vertices * 3)

    def forward(self, inputs):
        batch_size = inputs.shape[0]
        x = F.relu(self.fc1(inputs), inplace=True)
        x = F.relu(self.deconv1(x.view(-1, 512, 2, 2)), inplace=True)
        x = F.relu(self.deconv2(x), inplace=True)
        x = F.relu(self.deconv3(x), inplace=True)
        x = F.relu(self.fc2(x.view(-1, 23*23*64)), inplace=True)
...
class Model(nn.Module):
    def __init__(self, filename_obj, args):
        ...
        self.encoder = resnet.resnet18(num_classes=512)  # Encoder(im_size=args.image_size)
