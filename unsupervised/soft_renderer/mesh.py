...
class Mesh(object):
    ...
    def save_obj(self, filename_obj, idx=0, save_texture=False, texture_res_out=16):
        # if self.batch_size != 1:
        #     raise ValueError('Could not save when batch size >= 1')
        if save_texture:
            srf.save_obj(filename_obj, self.vertices[idx], self.faces[idx],
                         textures=self.textures[idx],
                         texture_res=texture_res_out, texture_type=self.texture_type)
        else:
            srf.save_obj(filename_obj, self.vertices[idx], self.faces[idx], textures=None)
